...
st.set_page_config(
    ...
    page_icon="🌿",
    layout="wide",
    initial_sidebar_state="collapsed",  # Cambiado para móviles
    menu_items={
        'Get Help': 'https://github.com/tu-usuario/agrodetect',
        'Report a bug': "mailto:tu-email@example.com",
        'About': "AgroDetect - Detección inteligente de enfermedades en plantas"
    }
)

# ── Estilos CSS personalizados (PWA optimizado) ───────────────────────────
st.markdown("""
<style>
...
.install-button {
    background: linear-gradient(135deg, #2d8a47, #3dba5f);
    color: white;
    border: none;
    padding: 1rem 2rem;
    border-radius: 12px;
    ...
    font-weight: 600;
    cursor: pointer;
    box-shadow: 0 4px 16px rgba(45, 138, 71, 0.4);
...
.install-button:hover {
    ...
    box-shadow: 0 6px 20px rgba(45, 138, 71, 0.5);
}
...
<meta name="apple-mobile-web-app-title" content="AgroDetect">
...
function installPWA() {
  if (!deferredPrompt) {
    alert('Esta app ya está instalada o el navegador no soporta instalación.');
    return;
  }
  ...
  deferredPrompt.userChoice.then((choiceResult) => {
    if (choiceResult.outcome === 'accepted') {
      console.log('✅ Usuario aceptó instalar AgroDetect');
    } else {
...
window.addEventListener('appinstalled', () => {
  console.log('🎉 AgroDetect instalado exitosamente');
  deferredPrompt = null;
  if (installButton) {
    installButton.classList.add('hidden');
  }
...
if (window.matchMedia('(display-mode: standalone)').matches) {
  console.log('📱 AgroDetect está corriendo como app instalada');
}
</script>

<!-- Botón de instalación (solo visible si no está instalada) -->
<div id="installButtonContainer" style="text-align: center; margin: 2rem 0;">
  <button 
    id="installBtn" 
    class="install-button hidden" 
    onclick="installPWA()"
  >
    📱 Instalar AgroDetect en tu celular
  </button>
</div>
